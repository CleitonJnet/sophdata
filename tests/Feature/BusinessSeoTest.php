...
test('business internal category pages expose specific seo metadata', function (
    ...
    ['/para-empresas/infraestrutura-corporativa-gerenciada/administracao-mensal', 'Administração Mensal | Infraestrutura Corporativa Gerenciada SophData'],
...
test('sitemap contains all required business urls without removing personal urls', function () {
    ...
    foreach ($businessUrls as $url) {
        expect($sitemap)->toContain(config('app.url').$url);
    }

    expect($sitemap)
        ->toContain(config('app.url').'/para-voce')
        ->toContain(config('app.url').'/para-voce/computador-lento')
        ->not->toContain(config('app.url').'/para-empresas/sites-e-sistemas');
});
