...
@section('meta_description', 'Soluções de TI para empresas com suporte, redes, backup, sites, sistemas e automação para resolver paradas, Wi-Fi instável e planilhas demais.')
...
@section('content')
    @php
        ...
        $featuredProblems = collect($problemOrder)
            ->map(fn (string $title) => $problemsByTitle->get($title))
            ->filter();
        $packageLevels = [
            [
                'title' => 'Essencial',
        ...
        $businessTypes = [
            'Igrejas', 'Escolas', 'Escritórios', 'Consultórios',
            'Lojas', 'Prestadores de serviço', 'Pequenas empresas', 'Instituições',
        ];
        $reasons = [
        ...
        $technologies = [
            'Linux', 'Laravel', 'PHP', 'MySQL', 'PostgreSQL', 'Java',
            'Spring Boot', 'Redes', 'Backup', 'Segurança Digital', 'Montagem de Computadores',
        ];
    @endphp
    ...
    <x-site.hero-banner
        ...
        :primary-button-text="$portal['primary_cta']"
        :primary-button-url="$whatsappUrl"
        :secondary-button-text="$portal['secondary_cta']"
        secondary-button-url="#solutions"
        :image="$portal['image']"
    ...
    <section id="solutions" class="scroll-mt-48 bg-white py-16 sm:py-20 lg:py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <x-site.section-heading
                ...
                description="Escolha o problema mais próximo da sua realidade e veja a solução indicada."
                centered
            />
            <div class="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($featuredProblems as $card)
                    <x-site.category-card
                        ...
                        :image="$card['image']"
                        :url="route('portal.business.category', $card['target_category_slug'])"
    ...
    <section class="bg-brand-50 py-16 sm:py-20 lg:py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <x-site.section-heading
                eyebrow="Categorias empresariais"
                title="Soluções organizadas por área"
                description="Escolha a área do problema e compare os pacotes disponíveis."
                centered
            />
            <div class="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                @foreach ($categories as $category)
                    <x-site.category-card
                        ...
                        :url="route('portal.business.category', $category['slug'])"
                        :items="array_slice($category['benefits'], 0, 3)"
                        cta-label="Ver pacotes"
                    />
                @endforeach
            </div>
        </div>
    ...
    <section class="bg-white py-16 sm:py-20 lg:py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <x-site.section-heading
                eyebrow="Progressão comercial"
                title="Escolha o nível de atendimento ideal"
    ...
    <section class="bg-slate-50 py-16 sm:py-20 lg:py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <x-site.section-heading
                ...
                title="Para quais negócios?"
                description="Atendimento para rotinas reais de pequenos negócios, equipes e instituições."
                centered
            />
            <div class="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($businessTypes as $businessType)
                    <article class="rounded-2xl border border-slate-200 bg-white p-5 text-center font-bold text-brand-950 shadow-sm">
                        {{ $businessType }}
                    </article>
                @endforeach
            </div>
    ...
    <section class="bg-white py-16 sm:py-20 lg:py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <x-site.section-heading
                ...
                title="Por que contratar a SophData?"
                description="Correção de problemas, organização da rotina e orientação em linguagem simples."
                centered
            />
    ...
    <section class="bg-brand-50 py-16 sm:py-20 lg:py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <x-site.section-heading
                ...
                title="Tecnologias e áreas"
                description="Suporte, redes, backup, sites, sistemas, dados e computadores para a rotina da empresa."
                centered
            />
            <div class="mt-10 flex flex-wrap justify-center gap-3">
                @foreach ($technologies as $technology)
                    <span class="rounded-full border border-brand-200 bg-white px-5 py-3 text-sm font-bold text-brand-800 shadow-sm">
                        {{ $technology }}
                    </span>
                @endforeach
            </div>
    ...
    <x-site.cta-section
        ...
        button-text="Solicitar atendimento empresarial"
        :button-url="$whatsappUrl"
        :image="config('sophdata.images.banner')"
