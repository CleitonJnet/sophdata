...
@php
    ...
    $hasRealLogo = is_string($symbolPath)
...
<a
    ...
    aria-label="Ir para o portal principal da SophData"
    {{ $attributes->class([
        'inline-flex items-center gap-2 rounded-xl',
        'bg-white px-3 py-2' => $light,
    ]) }}
>
    @if ($hasRealLogo)
        <img src="{{ asset($symbolPath) }}" alt="SophData" class="h-9 w-auto sm:h-10" decoding="async">
        <img src="{{ asset($wordmarkPath) }}" alt="" aria-hidden="true" class="hidden h-8 w-auto sm:block" decoding="async">
    @else
        ...
        <span @class(['font-bold', 'text-brand-950' => ! $light, 'text-brand-950' => $light])>SophData</span>
    @endif
</a>
