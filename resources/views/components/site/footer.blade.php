...
<footer class="bg-brand-950 text-white">
    <div class="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.2fr_.8fr_1fr_1fr_1fr] lg:px-8">
        ...
        <section>
            ...
            <nav class="mt-5 grid gap-1 text-sm text-brand-100/85" aria-label="Links do rodapé">
                <a href="{{ route('portal.business') }}" class="flex min-h-11 items-center hover:text-white">Para Empresas</a>
                <a href="{{ route('portal.personal') }}" class="flex min-h-11 items-center hover:text-white">Para Você</a>
                <a href="{{ route('portal.choose') }}" class="flex min-h-11 items-center hover:text-white">Escolher perfil</a>
                ...
                <a href="{{ route('site.privacy') }}" class="flex min-h-11 items-center hover:text-white">Política de Privacidade</a>
            </nav>
        </section>
        ...
        <section>
            ...
            <nav class="mt-5 grid gap-1 text-sm text-brand-100/85" aria-label="Principais soluções empresariais">
                @foreach ($businessSolutions as $solution)
                    <a href="{{ route('portal.business.category', $solution['slug']) }}" class="flex min-h-11 items-center hover:text-white">
                        {{ $solution['title'] }}
                    </a>
                @endforeach
            </nav>
        ...
        <section>
            ...
            <nav class="mt-5 grid gap-1 text-sm text-brand-100/85" aria-label="Principais soluções pessoais">
                @foreach ($personalSolutions as $solution)
                    <a href="{{ route('portal.personal.category', $solution['slug']) }}" class="flex min-h-11 items-center hover:text-white">
                        {{ $solution['title'] }}
                    </a>
                @endforeach
            </nav>
        ...
        <section>
            ...
            <div class="mt-5 grid gap-3 text-sm text-brand-100/85">
                ...
                <a href="mailto:{{ config('sophdata.brand.email') }}" class="break-all hover:text-white">{{ config('sophdata.brand.email') }}</a>
